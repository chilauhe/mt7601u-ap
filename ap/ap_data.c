...
VOID APSendPackets(NDIS_HANDLE dev_hnd, PPNDIS_PACKET pkt_list, UINT pkt_cnt)
{
	...
	for (Index = 0; Index < pkt_cnt; Index++) {
		...
		if (RTMP_GET_PACKET_NET_DEVICE(pPacket) >=
			...
			allowToSend =
			    ApWdsAllowToSendPacket(pAd, pPacket, &wcid);
		else
#endif				/* WDS_SUPPORT */
...
NDIS_STATUS APSendPacket(RTMP_ADAPTER * pAd, PNDIS_PACKET pPacket)
{
	...
	if (pMacEntry && (IS_ENTRY_WDS(pMacEntry) || IS_ENTRY_APCLI(pMacEntry)
...
static inline VOID APFindCipherAlgorithm(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
	...
	if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bWDSEntry)) {
		if (pAd->WdsTab.WdsEntry[pMacEntry->MatchWDSTabIdx].WepStatus ==
		    Ndis802_11Encryption1Enabled
		    || pAd->WdsTab.WdsEntry[pMacEntry->MatchWDSTabIdx].
			...
			if (pMacEntry->TxSndgType == SNDG_TYPE_SOUNDING) {
				...
				if (pAd->CommonCfg.ETxBfNoncompress == 0 &&
				    (pMacEntry->HTCapability.TxBFCap.ExpComBF >
				     0))
					((PHT_CONTROL)
					 pHeaderBufPtr)->CSISTEERING = 3;
				else
					((PHT_CONTROL)
					 pHeaderBufPtr)->CSISTEERING = 2;
...
VOID AP_AMPDU_Frame_Tx(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
...
#endif				/* VENDOR_FEATURE1_SUPPORT */
	{
		...
		*(pHeaderBufPtr + 1) = 0;
		...
		if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bSwEncrypt)) {
			 ...
			 */
			EXTRA_LLCSNAP_ENCAP_FROM_PKT_OFFSET(pTxBlk->pSrcBufData
							    - 2,
							    pTxBlk->
							    pExtraLlcSnapEncap);
	...
	if (pMacEntry && IS_ENTRY_WDS(pMacEntry)) {
		INC_COUNTER64(pAd->WdsTab.WdsEntry[pMacEntry->MatchWDSTabIdx].
			      WdsCounter.TransmittedFragmentCount);
		pAd->WdsTab.WdsEntry[pMacEntry->MatchWDSTabIdx].WdsCounter.
...
VOID AP_AMSDU_Frame_Tx(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
	...
	while (pTxBlk->TxPacketList.Head) {
...
#ifdef INCLUDE_DEBUG_QUEUE
		if (pAd->CommonCfg.DebugFlags & DBF_DBQ_TXFRAME)
			dbQueueEnqueueTxFrame((UCHAR *) (&pTxBlk->HeaderBuf
							 [TXINFO_SIZE]), NULL);
#endif				/* INCLUDE_DEBUG_QUEUE */
#endif				/* DBG_CTRL_SUPPORT */
		...
		if (pTxBlk->pMacEntry && IS_ENTRY_WDS(pTxBlk->pMacEntry)) {
			INC_COUNTER64(pAd->WdsTab.
				      WdsEntry[pTxBlk->pMacEntry->
					       MatchWDSTabIdx].WdsCounter.
				      TransmittedFragmentCount);
			pAd->WdsTab.WdsEntry[pTxBlk->pMacEntry->MatchWDSTabIdx].
...
VOID AP_Legacy_Frame_Tx(RTMP_ADAPTER * pAd, TX_BLK * pTxBlk)
{
	...
	if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bWMM)) {
		/* build QOS Control bytes */
		*pHeaderBufPtr =
		    ((pTxBlk->UserPriority & 0x0F) | (pAd->CommonCfg.AckPolicy
						      [pTxBlk->QueIdx] << 5));
#ifdef UAPSD_SUPPORT
		if (CLIENT_STATUS_TEST_FLAG
		    (pTxBlk->pMacEntry, fCLIENT_STATUS_APSD_CAPABLE)
	...
	if (pTxBlk->pMacEntry && IS_ENTRY_WDS(pTxBlk->pMacEntry)) {
		INC_COUNTER64(pAd->WdsTab.
			      WdsEntry[pTxBlk->pMacEntry->MatchWDSTabIdx].
			      WdsCounter.TransmittedFragmentCount);
		pAd->WdsTab.WdsEntry[pTxBlk->pMacEntry->MatchWDSTabIdx].
		    WdsCounter.TransmittedByteCount += pTxBlk->SrcBufLen;
	}
#endif				/* WDS_SUPPORT */
#endif				/* STATS_COUNT_SUPPORT */
...
/*	PQUEUE_HEADER   pQueue; */
	...
	if (P2P_CLI_ON(pAd)
		...
		DBGPRINT(RT_DEBUG_ERROR,
			 ("APHardTransmit fail pAd->LatchRfRegs.Channel=%d pAd->ApCliMlmeAux.Channel=%d pAd->ApCliMlmeAux.CentralChannel=%d\n",
			  pAd->LatchRfRegs.Channel, pAd->ApCliMlmeAux.Channel,
	...
	do {
		...
		if ((pHeader->FC.SubType == SUBTYPE_AUTH) &&
			...
			if (RTMPSoftDecryptWEP(pAd,
					       &pAd->SharedKey[pEntry->apidx]
					       [pRxWI->RxWIKeyIndex], pMgmt,
					       &mgmt_len) == FALSE) {
				DBGPRINT(RT_DEBUG_ERROR,
		...
		if (pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].
			...
			if (pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].
				...
				if (pAd->ApCfg.
					...
					pAd->ApCfg.ApCliTab
					    [pEntry->MatchAPCLITabIdx].SharedKey
					    [idx].CipherAlg = CipherAlg;
					pAd->ApCfg.ApCliTab
					    [pEntry->MatchAPCLITabIdx].SharedKey
					    [idx].KeyLen =
					    pAd->ApCfg.ApCliTab
	...
	if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 1)) {
		do {
			...
			/* handle WDS */
			{
				bWdsPacket = TRUE;
				if (MAC_ADDR_EQUAL
				    (pHeader->Addr1, pAd->CurrentAddress))
					pEntry =
					    FindWdsEntry(pAd,
							 pRxWI->
							 RxWIWirelessCliID,
...
//---Add by shiang for debug
		...
		if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1)) {
			if (MAC_ADDR_EQUAL(pHeader->Addr1, pAd->CurrentAddress))
				pEntry =
				    FindWdsEntry(pAd, pRxWI->RxWIWirelessCliID,
						 pHeader->Addr2,
						 pRxWI->RxWIPhyMode);

#ifdef STATS_COUNT_SUPPORT
			if (pEntry) {
				pAd->WdsTab.WdsEntry[pEntry->MatchWDSTabIdx].
				    WdsCounter.ReceivedByteCount +=
				    pRxWI->RxWIMPDUByteCnt;
				INC_COUNTER64(pAd->WdsTab.
					      WdsEntry[pEntry->MatchWDSTabIdx].
					      WdsCounter.ReceivedFragmentCount);

				if (IS_MULTICAST_MAC_ADDR(pHeader->Addr3))
					INC_COUNTER64(pAd->WdsTab.WdsEntry
						      [pEntry->MatchWDSTabIdx].
						      WdsCounter.
						      MulticastReceivedFrameCount);
		...
		case BTYPE_DATA:
			if (pRxInfo->U2M) {
				...
				if (pRxWI->RxWIMCS < 24) {	/* 3*3 */
					pAd->DiagStruct.RxDataCnt
					    [pAd->DiagStruct.ArrayCurIdx]++;
					pAd->DiagStruct.RxMcsCnt
					    [pAd->DiagStruct.ArrayCurIdx]
					    [pRxWI->RxWIMCS]++;
				}
#endif				/* DBG_DIAGNOSE */
			     ...
			     IN PNDIS_PACKET pRxPacket, IN RX_BLK * pRxBlk)
{
	...
	if ((pHeader->FC.FrDs == 1) && (pHeader->FC.ToDs == 1)) {
		if (MAC_ADDR_EQUAL(pHeader->Addr1, pAd->CurrentAddress))
			pEntry =
			    FindWdsEntry(pAd, pRxWI->RxWIWirelessCliID,
					 pHeader->Addr2, pRxWI->RxWIPhyMode);

#ifdef STATS_COUNT_SUPPORT
		if (pEntry) {
			pAd->WdsTab.WdsEntry[pEntry->MatchWDSTabIdx].WdsCounter.
			    ReceivedByteCount += pRxWI->RxWIMPDUByteCnt;
			INC_COUNTER64(pAd->WdsTab.
				      WdsEntry[pEntry->MatchWDSTabIdx].
				      WdsCounter.ReceivedFragmentCount);
