...
VOID MT7601_TssiDcGainCalibration(RTMP_ADAPTER * pAd)
{
	...
	for (i = 0; i < 4; i++) {
		...
		tssi_linear =
		    (tssi_linear & 0x80) ? tssi_linear - 0x100 : tssi_linear;
...
BOOLEAN MT7601_GetTssiCompensationParam(IN PRTMP_ADAPTER pAd,
				      ...
				      IN PCHAR pDeltaPowerByBbpR1)
{
	...
	if (pAd->Mlme.OneSecPeriodicRound % 4 == 0) {
		...
		if (MT7601_GetTssiCompensationParam
...
VOID MT7601SetRxAnt(IN PRTMP_ADAPTER pAd, IN UCHAR Ant)
{
	UCHAR Val;
	...
	if (Ant == 0) {
		...
		DBGPRINT(RT_DEBUG_OFF,
			 ("\x1b[31m%s: MT7601 --> switch to main\x1b[m\n",
			  __FUNCTION__));
	} else {
		RTMP_BBP_IO_READ8_BY_REG_ID(pAd, BBP_R152, &Val);
		Val |= 0x80;
		...
		DBGPRINT(RT_DEBUG_OFF,
			 ("\x1b[31m%s: MT7601 --> switch to aux\x1b[m\n",
			  __FUNCTION__));
	}
}
