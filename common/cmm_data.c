	...
	if (pTxBlk->Wcid == MCAST_WCID) {
		...
		if (pTxBlk->TxFrameType == TX_LEGACY_FRAME) {
			...
			if ((IS_HT_RATE(pMacEntry) == FALSE) &&
			    (CLIENT_STATUS_TEST_FLAG
			     (pMacEntry, fCLIENT_STATUS_PIGGYBACK_CAPABLE)))
	...
	for (QueIdx = sQIdx; QueIdx <= eQIdx; QueIdx++) {
		...
		while (1) {
			...
			if (RTMP_GET_PACKET_MGMT_PKT(pPacket) == 1) {
				...
				status =
				    MlmeHardTransmitMgmtRing(pAd, QueIdx,
							     pPacket);
				...
				IN ULONG DataSize, IN UCHAR OpMode)
{
	...
	if ((FromWhichBSSID < pAd->ApCfg.BssidNum)
		...
		VLAN_Size =
		    (pAd->WdsTab.WdsEntry[FromWhichBSSID -
